...
@section('content')
    <div class="container-fluid p-4">
                    ...
                    <div class="d-flex gap-2 align-items-center">
                        ...
                        <form action="{{ route('admin.auth.users.toggle-sharding') }}" method="POST" class="d-inline">
                            ...
                            <div class="form-check form-switch d-flex align-items-center gap-2" style="margin-bottom: 0;">
                                <input class="form-check-input" type="checkbox" id="shardingToggle"
                                    {{ $shardingEnabled ? 'checked' : '' }} onchange="this.form.submit()"
                                    style="cursor: pointer; width: 48px; height: 24px;">
                                <label class="form-check-label" for="shardingToggle"
        ...
        <section class="row mb-4">
            <!-- Total Users Stats -->
            <!-- Total Users Stats -->
            <div class="col-xl-3 col-lg-6 col-md-12 col-12 mb-4 mb-xl-0">
                <div class="card h-100">
                    <div class="card-body">
                        <div class="d-flex align-items-center justify-content-between mb-3">
                            <div>
                                <h4 class="mb-0">전체 회원수</h4>
                            </div>
                        ...
                        <div class="d-flex align-items-center mb-2">
                            <h2 class="fw-bold fs-2 mb-0 me-2">{{ number_format($totalUsers) }}</h2>
                            <span class="text-muted">명</span>
                        </div>
                        <div class="ms-1">
                            <small class="text-muted">시스템에 등록된 총 사용자 수</small>
                        </div>
                    </div>
                </div>
            </div>
                ...
                <div class="card h-100">
                    <div class="card-body">
                        ...
                        @php
                            ...
                            $totalCount = $adminCount + $userCount + ($roleStats['other'] ?? 0);
                            $adminPercent = $totalCount > 0 ? ($adminCount / $totalCount) * 100 : 0;
                            $userPercent = $totalCount > 0 ? ($userCount / $totalCount) * 100 : 0;
                        @endphp
                        ...
                        <div class="d-flex flex-column gap-2">
                            ...
                            <div>
                                <div class="d-flex justify-content-between align-items-center mb-1">
                                    <span class="text-muted fw-semi-bold">관리자</span>
                                    <span class="fw-bold">{{ number_format($adminCount) }}명
                                        ({{ round($adminPercent, 0) }}%)</span>
                                </div>
                                <div class="progress" style="height: 4px;">
                                    <div class="progress-bar bg-danger" role="progressbar"
                                        style="width: {{ $adminPercent }}%"></div>
                                </div>
                            </div>
                            <!-- User -->
                            <div>
                                <div class="d-flex justify-content-between align-items-center mb-1">
                                    <span class="text-muted fw-semi-bold">일반사용자</span>
                                    <span class="fw-bold">{{ number_format($userCount) }}명
                                        ({{ round($userPercent, 0) }}%)</span>
                                </div>
                                <div class="progress" style="height: 4px;">
            ...
            @if ($shardingEnabled && isset($shardStatistics['shards']))
                <div class="col-xl-6 col-lg-12 col-md-12 col-12">
                    <div class="card h-100">
                        <div class="card-header border-bottom-0 pb-0">
                            <h4 class="mb-0">샤드별 회원 분포</h4>
                        </div>
                            ...
                            <div class="w-100">
                                <div class="row">
                                    @foreach ($shardStatistics['shards'] as $shard)
                                        @php
                                            $sCount = $shard['user_count'] ?? 0;
                                            $sPercent = $totalUsers > 0 ? ($sCount / $totalUsers) * 100 : 0;
                                            $sColor = $loop->iteration % 2 == 0 ? 'info' : 'primary';
                                            // You can rotate colors if desired
                                        @endphp
                                        <div class="col-md-6 col-lg-6 mb-3">
                                            <span class="d-block text-muted fw-semi-bold mb-1">
                                                {{ $shard['table_name'] }}
                                            </span>
                                            <div class="d-flex align-items-end mb-2">
                                                <h3 class="fw-bold mb-0 me-2">{{ number_format($sCount) }}명</h3>
                                                <span class="text-muted mb-1">({{ round($sPercent, 1) }}%)</span>
                                            </div>
                                            <div class="progress" style="height: 6px;">
                                                <div class="progress-bar bg-{{ $sColor }}" role="progressbar"
                                                    style="width: {{ $sPercent }}%"
                                                    aria-valuenow="{{ $sPercent }}" aria-valuemin="0"
                                ...
                                <div class="row">
                                    <div class="col-md-4">
                                        <input type="search" name="search" class="form-control"
                                            placeholder="이름, 이메일, 사용자명 검색..." value="{{ request('search') }}" />
                                    </div>
                                    <div class="col-md-8">
                                        <div class="d-flex gap-2 justify-content-end">
                                            ...
                                            <select name="status" class="form-select w-auto">
                                                ...
                                                <option value="active"
                                                    {{ request('status') == 'active' ? 'selected' : '' }}>활성</option>
                                                <option value="inactive"
                                                    {{ request('status') == 'inactive' ? 'selected' : '' }}>비활성</option>
                                                <option value="suspended"
                                                    {{ request('status') == 'suspended' ? 'selected' : '' }}>정지</option>
                                            </select>
                                ...
                                <tbody>
                                    @foreach ($shardStatistics['shards'] as $shard)
                                        <tr>
                                            ...
                                            <td>
                                                @if ($shard['exists'])
                                                    <a href="?shard_id={{ $shard['shard_id'] }}"
                                            ...
                                            <td>
                                                @if ($shard['exists'])
                                                    @php
                                                        ...
                                                        $percentage =
                                                            $totalUsers > 0
                                                                ? round(($userCount / $totalUsers) * 100, 1)
                                                                : 0;
                                                    @endphp
                                                    <div>
                                                        <div class="d-flex align-items-center gap-2 mb-1">
                                                            <span
                                                                class="text-dark fw-bold">{{ number_format($userCount) }}명</span>
                                                        ...
                                                        <div class="d-flex gap-2">
                                                            <small class="text-success">
                                                                <i class="fe fe-check-circle"></i> 활성
                                                                {{ number_format($activeCount) }}
                                                            </small>
                                                            @if ($inactiveCount > 0)
                                                                <small class="text-warning">
                                                                    <i class="fe fe-x-circle"></i> 비활성
        ...
        @else
            ...
            @if ($shardingEnabled && request('shard_id'))
                <div class="alert alert-info alert-dismissible fade show">
                    <i class="fe fe-info me-2"></i>
                    샤드 {{ request('shard_id') }}
                    ({{ $shardStatistics['shards'][request('shard_id') - 1]['table_name'] ?? '' }}) 사용자 목록
                                        ...
                                        <div class="d-flex gap-2 justify-content-end">
                                            <select name="role" class="form-select w-auto">
                                                ...
                                                <option value="admin" {{ request('role') == 'admin' ? 'selected' : '' }}>
                                                    관리자</option>
                                                <option value="editor"
                                                    {{ request('role') == 'editor' ? 'selected' : '' }}>편집자</option>
                                                <option value="user" {{ request('role') == 'user' ? 'selected' : '' }}>
                                                    사용자</option>
                                            </select>
                                            <select name="status" class="form-select w-auto">
                        ...
                        <div class="table-responsive">
                            <table class="table mb-0 text-nowrap table-hover table-centered">
                                ...
                                <tbody>
                                        ...
                                        <tr>
                                            ...
                                            <td>
                                                <div class="d-flex align-items-center flex-row gap-2">
                                                    @php
                                                        ...
                                                        $avatarUrl =
                                                            route('admin.user-avatar.index', $user->id) . $shardQuery;
                                                    @endphp
                                                    <a href="{{ $avatarUrl }}" class="text-decoration-none"
                                                        data-bs-toggle="tooltip" title="아바타 관리">
                                                        @if ($user->avatar ?? false)
                                                            <img src="{{ $user->avatar }}" alt="{{ $user->name }}"
                                                                class="rounded-circle avatar-md" style="cursor: pointer;"
                                                    ...
                                                    <div>
                                                        ...
                                                        <small
                                                            class="text-muted">{{ '@' . ($user->username ?? 'N/A') }}</small>
                                                    </div>
                                                </div>
                                            </td>
                                            <td>
                                                @php
                                                    ...
                                                    $emailShardQuery = $emailShardParam
                                                        ? '?shard_id=' . $emailShardParam
                                                        : '';
                                                @endphp
                                                <a href="{{ route('admin.auth.users.show', $user->id) }}{{ $emailShardQuery }}"
                                            ...
                                            <td>
                                                <div class="hstack gap-2">
                                                    ...
                                                    <form
                                                        action="{{ route('admin.auth.users.destroy', $user->id) }}{{ $shardQuery }}"
                                                        method="POST" class="d-inline"
                                                        onsubmit="return confirm('정말로 이 사용자를 삭제하시겠습니까?');">
                                                        @csrf
                                                        @method('DELETE')
                                                        @if ($shardParam)
                                                            <input type="hidden" name="shard_id"
                                                                value="{{ $shardParam }}">
                                                        @endif
                                    ...
                                    @empty
                                        <tr>
                                            <td colspan="{{ $shardingEnabled && !request('shard_id') && request('search') ? 9 : 8 }}"
