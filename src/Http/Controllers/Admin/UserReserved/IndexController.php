...
use Jiny\Auth\Models\UserReserved;
...
class IndexController extends Controller
{
    ...
    protected function loadConfig()
    {
        ...
        $this->config = [
            'view' => $indexConfig['view'] ?? 'jiny-auth::admin.user-reserved.index',
            'title' => $indexConfig['title'] ?? '예약 키워드 관리',
            'subtitle' => $indexConfig['subtitle'] ?? '예약된 키워드 목록',
    ...
    public function __invoke(Request $request)
    {
        ...
        // 페이지네이션
        $reserved = $query->paginate($this->config['per_page'])->withQueryString();
