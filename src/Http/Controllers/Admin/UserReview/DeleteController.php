<?php

namespace Jiny\Auth\Http\Controllers\Admin\UserReview;

use App\Http\Controllers\Controller;

/**
 * 관리자 - 사용자 리뷰 삭제 처리 컨트롤러
 *
 * 진입 경로:
 * Route::delete('/admin/auth/user/reviews/{id}') → DeleteController::__invoke()
 */
class DeleteController extends Controller
{
    protected $actions;

    public function __construct()
    {
        $this->middleware(['auth', 'admin']);
        $this->loadActions();
    }

    /**
     * JSON 설정 파일 로드
     */
    protected function loadActions()
    {
        $configPath = __DIR__ . '/UserReview.json';
        $jsonConfig = json_decode(file_get_contents($configPath), true);

        $deleteConfig = $jsonConfig['delete'] ?? [];

        $this->actions = [
            'routes' => [
                'success' => $deleteConfig['redirect']['success'] ?? 'admin.auth.user.reviews.index',
            ],
            'messages' => [
                'success' => $deleteConfig['messages']['success'] ?? '리뷰가 성공적으로 삭제되었습니다.',
                'error' => $deleteConfig['messages']['error'] ?? '리뷰 삭제에 실패했습니다.',
            ],
        ];
    }

    /**
     * 사용자 리뷰 삭제 처리
     */
    public function __invoke($id)
    {
        $review = \DB::table('user_reviews')->where('id', $id)->first();

        if (!$review) {
            return redirect()->route('admin.auth.user.reviews.index')
                ->with('error', '리뷰를 찾을 수 없습니다.');
        }

        \DB::table('user_reviews')->where('id', $id)->delete();

        return redirect()
            ->route($this->actions['routes']['success'])
            ->with('success', $this->actions['messages']['success']);
    }
}
