...
class IndexController extends Controller
{
    ...
    protected function loadConfig()
    {
        ...
        $this->config = [
            'view' => $indexConfig['view'] ?? 'jiny-auth::admin.user-phone.index',
            'title' => $indexConfig['title'] ?? '사용자 전화번호 관리',
            'subtitle' => $indexConfig['subtitle'] ?? '사용자 전화번호 목록',
    ...
    public function __invoke(Request $request)
    {
        ...
        // 페이지네이션
        $phones = $query->paginate($this->config['per_page'])->withQueryString();

        return view($this->config['view'], compact('phones'));
    }
}