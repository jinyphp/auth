...
use Jiny\Auth\Models\UserGrade;

/**
 * 관리자 - 사용자 등급 목록 컨트롤러
 *
 * 진입 경로:
 * Route::get('/admin/auth/user/grades') → IndexController::__invoke()
 */
class IndexController extends Controller
{
    ...
    protected function loadConfig()
    {
        ...
        $this->config = [
            'view' => $indexConfig['view'] ?? 'jiny-auth::admin.user-grades.index',
            'title' => $indexConfig['title'] ?? '사용자 등급 관리',
            'subtitle' => $indexConfig['subtitle'] ?? '시스템 사용자 등급 목록',
    ...
    public function __invoke(Request $request)
    {
        ...
        // 페이지네이션
        $grades = $query->paginate($this->config['per_page'])->withQueryString();

        return view($this->config['view'], compact('grades'));
    }
}