 ...
 * 회원 가입 처리
 */
class RegistCreate extends Controller
{
    use Notifiable;
    ...
    public function store(Request $request)
    {
        ...
        */
        ...
        if($user = $this->isDatabaseUser($request->email)) {
            ...
            return view("jiny-auth::regist.error.layout",[
    ...
    private function checkPassword($password)
    {
        ...
        if(isset($this->setting['password']['min'])) {
            if($this->setting['password']['min'] > 0) {
        ...
        if(isset($this->setting['password']['max'])) {
            if($this->setting['password']['max'] > 0) {
        ...
        // 특수문자 포함여부 체크
        if(isset($this->setting['password']['special']) && $this->setting['password']['special']) {
        ...
        // 숫자 포함 체크
        if(isset($this->setting['password']['number']) && $this->setting['password']['number']) {
        ...
        // 영문자 포함 체크
        if(isset($this->setting['password']['alpha']) && $this->setting['password']['alpha']) {
    ...
    private function viewSuccess()
    {
        if(isset($this->setting['regist']['success_view'])) {
            if($this->setting['regist']['success_view']) {
                return $this->setting['regist']['success_view'];
            }
        }

        return "jiny-auth::login.success.layout";
    }
